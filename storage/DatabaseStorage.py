    ...
    def save_result_hash(self, hash_key, result, started=None, endtime=None):
        ...
        if precheck_result is not None:
            if started is not None:
                DBUtil.execute_command(self._db_name, "UPDATE Results SET started = ?, result = ? WHERE name = ?", (started, hash_key))
            if endtime is not None:
                runtime = endtime - precheck_result[0]
                print "RUNTIME: " + str(runtime)
                print "HASHKEY: " + str(hash_key)
                DBUtil.execute_command(self._db_name, "UPDATE Results SET runtime = ?, result = ? WHERE name = ?", (str(runtime), result, str(hash_key)))
